...
- **`BaseGameState`** is the synced state every table has. Each game extends it — a Colyseus schema can't be shaped conditionally, so one state carrying both games' fields would have every Shelem client syncing Hokm's. `teamOfSeat` lives here as real state rather than a `seat % 2` derivation, so the server stays the single place that decides who is on whose side — Hokm's `aceDealTeams` picks the pairing from the cards and then physically reseats those two players opposite each other.
